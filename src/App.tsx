...
const initialTasks: Task[] = [
  { id: 1, time: "8:00 AM", title: "Morning routine", detail: "Breakfast, vitamins, and backpack check", done: true, completedBy: "David" },
  { id: 2, time: "12:30 PM", title: "Quiet reset", detail: "10 minutes with the breathing card", done: true, completedBy: "Sofia" },
...
const initialUpdates: Update[] = [
  { id: 1, author: "Sofia", role: "Aunt & caregiver", initials: "SO", time: "Today, 1:10 PM", body: "The quiet reset worked really well today. Elio picked the breathing card himself and returned to lunch calmly.", tone: "sage" },
  { id: 2, author: "David", role: "Dad", initials: "DP", time: "Today, 8:24 AM", body: "Morning routine is done. Backpack and reading folder are by the door for pickup.", tone: "blue" },
  { id: 3, author: "Maya", role: "Caregiver", initials: "MA", time: "Yesterday, 6:40 PM", body: "We reached the three-step bedtime routine without reminders. That makes four successful evenings this week!", tone: "peach" },
]
...
export default function App() {
  ...
  const [viewer, setViewer] = useState<"David" | "Maya">("David")
  ...
  const viewerRole = viewer === "David" ? "Parent" : "Caregiver"
  ...
  function addUpdate(event: FormEvent<HTMLFormElement>) {
    ...
    setUpdates((current) => [{
      ...
      initials: viewer === "David" ? "DP" : "MA", time: "Just now", body,
      tone: viewer === "David" ? "blue" : "peach",
    }, ...current])
    setNote("")
  }
    ...
    <Tabs defaultValue="today" orientation="vertical" className="app-shell">
      <aside className="app-sidebar">
        ...
        <div className="child-mini-card">
          ...
          <div><p className="eyebrow">Care space</p><p className="child-mini-name">Elio&apos;s circle</p></div>
        ...
        <div className="privacy-note">
          <ShieldCheck />
          <div><strong>Private care circle</strong><span>Only invited caregivers can see Elio&apos;s information.</span></div>
        </div>
      ...
      <main className="app-main">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <div className="viewer-switcher">
              <span className={`mini-avatar ${viewer === "David" ? "blue" : "peach"}`}>{viewer === "David" ? "DP" : "MA"}</span>
              <label><span>Viewing as</span><select value={viewer} onChange={(event) => setViewer(event.target.value as "David" | "Maya")}><option value="David">David · Parent</option><option value="Maya">Maya · Caregiver</option></select></label>
            </div>
          </div>
        </header>
          ...
          <section className="today-grid">
            <article className="daily-card">
              <div className="daily-copy">
                ...
                <p>{completed} of {tasks.length} care items are complete. Everyone in Elio&apos;s circle can see what comes next.</p>
                <div className="circle-people" aria-label="Care circle members"><span className="mini-avatar blue">DP</span><span className="mini-avatar peach">MA</span><span className="mini-avatar sage">SO</span><span className="people-label">3 caregivers active today</span></div>
              </div>
          ...
          <section className="content-grid">
            ...
            <div className="side-stack">
              <article className="panel goal-spotlight">
                ...
                <p>Elio completed 5 of 7 transitions this week with one prompt or less.</p><Progress value={68} className="goal-progress" />
                <div className="progress-label"><span>Weekly goal</span><strong>5 / 7</strong></div>
              </article>
              <article className="panel handoff-card">
                <div className="panel-heading compact"><div><p className="eyebrow">Latest handoff</p><h3>From Sofia</h3></div><span className="mini-avatar sage">SO</span></div>
        ...
        <TabsContent value="care" className="page-content">
          ...
          <article className="panel intervention-panel">
            ...
            <div className="intervention-steps"><div><span>1</span><strong>Pause</strong><p>Offer a calm ten-second pause before giving another direction.</p></div><div><span>2</span><strong>Name</strong><p>Help Elio identify the feeling or need using words or the visual card.</p></div><div><span>3</span><strong>Choose</strong><p>Offer two clear next-step options and let him choose.</p></div></div>
          </article>
        </TabsContent>
        ...
        <TabsContent value="progress" className="page-content">
          ...
          <section className="progress-summary-grid">
            ...
            <article className="panel wins-card"><div className="wins-icon"><Sparkles /></div><p className="eyebrow">This month</p><h3>18 meaningful wins</h3><p>Caregivers recorded successful routines, clear communication, and independent transitions.</p><div className="win-people"><span className="mini-avatar blue">DP</span><span>David logged 7</span><span className="mini-avatar peach">MA</span><span>Maya logged 6</span><span className="mini-avatar sage">SO</span><span>Sofia logged 5</span></div></article>
          </section>
        ...
        <TabsContent value="updates" className="page-content">
          <section className="section-intro"><div><p className="eyebrow">Caregiver updates</p><h2>One clear place for every handoff</h2><p>Share helpful context without losing it in texts or group chats.</p></div><div className="member-stack" aria-label="Four care circle members"><span className="mini-avatar blue">DP</span><span className="mini-avatar peach">MA</span><span className="mini-avatar sage">SO</span><span className="mini-avatar ink">+1</span></div></section>
          <section className="updates-layout">
            <div>
              <form className="panel update-composer" onSubmit={addUpdate}><span className={`mini-avatar ${viewer === "David" ? "blue" : "peach"}`}>{viewer === "David" ? "DP" : "MA"}</span><label><span className="sr-only">Share an update</span><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="Share a care update, observation, or handoff note…" rows={3} /></label><Button type="submit" disabled={!note.trim()}><MessageCircle /> Share update</Button></form>
            ...
            <aside className="panel circle-panel"><p className="eyebrow">Elio&apos;s care circle</p><h3>Everyone stays informed</h3><div className="circle-member"><span className="mini-avatar blue">DP</span><div><strong>David</strong><small>Parent · Full access</small></div><span className="online-dot" /></div><div className="circle-member"><span className="mini-avatar peach">MA</span><div><strong>Maya</strong><small>Caregiver · Daily care</small></div><span className="online-dot" /></div><div className="circle-member"><span className="mini-avatar sage">SO</span><div><strong>Sofia</strong><small>Aunt · Daily care</small></div></div><div className="circle-member"><span className="mini-avatar ink">JP</span><div><strong>Jordan</strong><small>Parent · Full access</small></div></div><Button variant="outline" className="invite-button"><Users /> Manage care circle</Button></aside>
          </section>
        </TabsContent>
      </main>
    </Tabs>
